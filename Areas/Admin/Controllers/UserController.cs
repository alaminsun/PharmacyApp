...
namespace PhramacyApp.Areas.Admin.Controllers
{
    ...
    public class UserController : BaseController<UserController>
    {
        ...
        //public static  UserViewModel ToDto(this ApplicationUser user)
        //{
        //    if (user != null)
        //    {
        ...
        public async Task<IActionResult> LoadAll()
        {
            ...
            var model = allUsersExceptCurrentUser;
            foreach (var exceptCurrentUser in model)
            {
                UserViewModel user = new UserViewModel();

                user.Id = exceptCurrentUser.Id;
                user.FirstName = exceptCurrentUser.FirstName;
                user.LastName = exceptCurrentUser.LastName;
                user.ProfilePicture = exceptCurrentUser.ProfilePicture;
                user.IsActive = exceptCurrentUser.IsActive;
                userList.Add(user);
            }
        ...
        public async Task<IActionResult> OnPostCreate(UserViewModel userModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var currentUser = await _userManager.GetUserAsync(HttpContext.User);
                    var allUsersExceptCurrentUser = await _userManager.Users.Where(a => a.Id != currentUser.Id).ToListAsync();
                    var users = allUsersExceptCurrentUser;
